...
public class DDSCompressor {
            ...
            for (int X = 0; X < W; X++) {
                ...
                for (int TY = 0; TY < 4; TY++) {
                    for (int TX = 0; TX < 4; TX++) {
                        ...
                        byte RedPx = (byte) Math.Min(colorTableR[(channelKeyR >> (TY * 12 + TX * 3)) & 7] * 255,
                            255);

                        byte AlphaPx = (byte) Math.Min(colorTableA[(channelKeyA >> (TY * 12 + TX * 3)) & 7] * 255,
                            255);
    ...
    public static int Get16(byte[] Data, int Address) {
        return
            Data[Address + 0] << 0 | Data[Address + 1] << 8;
    }

    public static int Get32(byte[] Data, int Address) {
        return
            Data[Address + 0] << 0 | Data[Address + 1] << 8 | Data[Address + 2] << 16 | Data[Address + 3] << 24;
    }
